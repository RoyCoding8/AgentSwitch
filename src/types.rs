...
pub enum ItemKind { Skill, Hook, Rule, Agent, Mcp, InstructionFile, SteeringRule, Spec }

impl ItemKind {
    pub fn label(self) -> &'static str {
        match self {
            Self::Skill => "Skills", Self::Hook => "Hooks", Self::Rule => "Rules",
            Self::Agent => "Agents", Self::Mcp => "MCP", Self::InstructionFile => "Files",
            Self::SteeringRule => "Steering", Self::Spec => "Specs",
        }
    }
}
...
impl ProviderId {
    pub const ALL: &[ProviderId] = &[Self::Claude, Self::Codex, Self::Gemini, Self::Kiro, Self::OpenCode];
    pub fn label(self) -> &'static str {
        match self {
            Self::Claude => "Claude Code", Self::Codex => "Codex CLI",
            Self::Gemini => "Gemini CLI", Self::Kiro => "Kiro", Self::OpenCode => "OpenCode",
        }
    }
...
impl ConfigItem {
    pub fn new(name: impl Into<String>, kind: ItemKind, path: PathBuf, provider: ProviderId) -> Self {
        ...
        Self { name: name.into(), kind, state, path, provider, editable, hook_loc: None }
    }
    ...
    pub fn enabled_path(&self) -> PathBuf {
        ...
        if let Some(base) = s.strip_suffix(".disabled") { PathBuf::from(base) } else { self.path.clone() }
    }
}
